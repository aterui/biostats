...
One potential approach is to utilize the Coefficient of Determination (Chapter \@ref(regression)) or its variation (Chapter \@ref(the-glm-framework)) as a measure of model performance. These metrics provide an indication of the proportion of variation in the response variable that is explained by the model, making them reasonable options. However, there are situations where these measures are not meaningful for model selection.
...
The mathematical details of AIC are beyond the scope of this discussion, but for those interested, reference Burnham and Anderson 2002. However, **it is crucial to understand that the AIC differs fundamentally from other measures as it assesses the model's robustness when new data is added.**
...
The proper application of AIC in biology has only been recently discussed, and there is often misuse of this metric, particularly in overlooking the second component. Therefore, it is crucial to clearly define the objective of your analysis to ensure the appropriate choice of statistical methods.
